...
/*   Updated: 2019/12/29 02:20:08 by oouklich         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				w3d_close(t_wolf3d *p)
{
	...
	i = -1;
	while (++i < TOTAL_TXT)
		mlx_destroy_image(p->mlx, p->txt.img[i]);
	if (p->map)
		free(p->map);
	mlx_destroy_image(p->mlx, p->m_img);
...
static void		w3d_init(t_wolf3d *p, char *file)
{
	...
	if (w3d_load_texture(p) == -1 && write(1, "error: textures fails\n", 22))
		exit(EXIT_FAILURE);
	p->dir = (t_vec2d){0.0, 1.0};
	p->plane = (t_vec2d){-0.66, 0.0};
...
int				main(int argc, char **argv)
{
	...
	w3d_init(&p, argv[1]);
	w3d_draw(&p);
	mlx_hook(p.win, 17, (1 << 17), w3d_close, &p);
